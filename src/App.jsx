...
function ServicesSection({ T }) {
  return (
    <section className="py-6">
      <h1 className={`text-4xl md:text-5xl font-['Playfair_Display'] uppercase tracking-[0.08em] ${T.navTextStrong}`}>Services</h1>
      <p className={`mt-2 ${T.muted}`}>Multi-genre coverage designed around your brief.</p>
      <div className="mt-6 grid md:grid-cols-2 xl:grid-cols-3 gap-6">
        {[
          ["Portraits & Headshots", ["60–90 min · up to 2 outfits", "Natural retouching", "Wardrobe/posing guidance"]],
          ["Fashion / Editorial", ["Moodboard planning", "Lighting & styling coordination", "Half-day / full-day"]],
          ["Events & Candids", ["Hourly/session blocks", "Key moments & people", "Colour-graded selects"]],
        ].map(([title, lines]) => (
          <article key={title} className={`rounded-2xl border p-5 shadow-sm ${T.panelBg} ${T.panelBorder}`}>
            <h3 className={`text-lg font-medium ${T.navTextStrong}`}>{title}</h3>
...
function PricingSection({ T }) {
  const tiers = [
    { name: "Portrait Session", price: "from ₹4,500", includes: ["60–90 min · up to 2 outfits", "6 lightly retouched hero shots", "Curated 3 - 5 edits/outfit"] },
    { name: "Headshots (Solo/Team)", price: "from ₹5,000", includes: ["Consistent lighting & framing", "On-location option", "Curated 3 - 5 edits/outfit"] },
    { name: "Fashion / Editorial (Half-day)", price: "from ₹10,000", includes: ["Pre-prod & moodboard", "Lighting & looks", "Team coordination on request"] },
    { name: "Event Coverage (2 hrs)", price: "from ₹8,000", includes: ["Focused coverage", "Colour-graded selects", "Extendable by hour"] },
  ];
  return (
    <section className="py-6">
      <h1 className={`text-4xl md:text-5xl font-['Playfair_Display'] uppercase tracking-[0.08em] ${T.navTextStrong}`}>Pricing (indicative)</h1>
      <p className={`mt-2 ${T.muted}`}>Final quote depends on scope, locations, team and timelines.</p>
      <div className="mt-6 grid md:grid-cols-2 xl:grid-cols-3 gap-6">
        {tiers.map((t) => (
          <article key={t.name} className={`rounded-2xl border p-5 shadow-sm ${T.panelBg} ${T.panelBorder}`}>
            ...
            <a href="#/contact" className={`${T.link} text-sm mt-4 inline-block`} onClick={() => trackEvent("nav_click", { to: "contact_from_pricing" })}>
              Request a quote →
            </a>
          </article>
        ))}
