  ...
  return (
    <section id="home" className="relative min-h-[68vh] md:min-h-[78vh]">
      ...
      <img
        ...
        loading="eager"
      />
      {/* Dark veil for readability on top of photo */}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {loading ? (
          <span className={`text-xs ${T.muted2}`}>Loading…</span>
        ) : null}
      </div>
      ...
      {error ? (
        ...
        <div className={`px-4 py-6 text-sm ${T.muted}`}>
          No images found in <strong className={T.navTextStrong}>{label}</strong> yet.
          Upload JPG/PNG/WebP to{" "}
      ...
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <p className={`mt-1 ${T.muted}`}>
            Images load straight from your GitHub repo folders.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {tags.map((t) => {
            const isActive = active === t;
            const activeCls = T.chipActive;
            const inactiveCls = T.chipInactive;
            return (
              <button
                key={t}
                onClick={() => setActive(t)}
                className={`px-3 py-1.5 rounded-full text-sm border transition ${isActive ? activeCls : inactiveCls}`}
...
function BookingSection({ T }) {
  ...
  const onSubmit = (e) => {
    ...
    const subject = encodeURIComponent(
      `Shoot enquiry: ${form.service} — ${form.name}`
    );
          ...
          >
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  type="submit"
                  className="rounded-xl bg-neutral-900 text-white px-4 py-2 font-medium hover:opacity-90"
                >
                  Send Enquiry
                </button>
                <button
        ...
        <nav className={`${CONTAINER} flex items-center justify-between`}>
          ...
          <ul className="hidden md:flex items-center gap-1 text-sm">
            {NAV_ITEMS.map(({ label, id, icon }) => (
              <li key={id}>
                <button
                  className={`relative group flex items-center gap-2 px-3 py-2 rounded-lg ${T.navText} focus:outline-none focus-visible:ring-2 focus-visible:ring-black/10`}
                  onClick={() => scrollTo(id)}
                >
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Mobile menu toggle */}
            <button
              className={`md:hidden rounded-lg px-3 py-2 text-sm border ${T.btnOutline}`}
